  ...
  const double dist_thr, const rclcpp::Logger logger)
{
  ...
  int temp;
  if (!planning_utils::calcClosestIndex(path, stop_point_from_map, temp, dist_thr)) {
...
bool generateStopLineBeforeIntersection(
  [[maybe_unused]] const int lane_id, [[maybe_unused]] lanelet::LaneletMapConstPtr lanelet_map_ptr,
  [[maybe_unused]] const std::shared_ptr<const PlannerData> & planner_data,
  [[maybe_unused]] const autoware_auto_planning_msgs::msg::PathWithLaneId & input_path,
  [[maybe_unused]] autoware_auto_planning_msgs::msg::PathWithLaneId * output_path,
  [[maybe_unused]] int * stuck_stop_line_idx, [[maybe_unused]] int * pass_judge_line_idx,
