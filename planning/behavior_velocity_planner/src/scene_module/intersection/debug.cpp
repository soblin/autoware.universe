...
visualization_msgs::msg::MarkerArray IntersectionModule::createVirtualWallMarkerArray()
{
  ...
  if (debug_data_.collision_stop_wall_pose) {
    appendMarkerArray(
      motion_utils::createStopVirtualWallMarker(
        debug_data_.collision_stop_wall_pose.value(), "intersection", now, uid),
      &wall_marker, now);
    uid++;
  }
  if (debug_data_.occlusion_first_stop_wall_pose) {
    appendMarkerArray(
      motion_utils::createStopVirtualWallMarker(
        debug_data_.occlusion_first_stop_wall_pose.value(), "intersection", now, uid),
      &wall_marker, now);
    uid++;
  }
  if (debug_data_.occlusion_stop_wall_pose) {
    appendMarkerArray(
      motion_utils::createStopVirtualWallMarker(
        debug_data_.occlusion_stop_wall_pose.value(), "intersection_occlusion", now, module_id_),
      &wall_marker, now);
    uid++;
  }
