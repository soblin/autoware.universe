...
visualization_msgs::msg::MarkerArray IntersectionModule::createDebugMarkerArray()
{
  ...
  appendMarkerArray(
    createPolygonMarkerArray(
      debug_data_.intersection_area, "intersection_area", lane_id_, 0.3, 0.0, 0.0),
    current_time, &debug_marker_array);

  appendMarkerArray(
    createPolygonMarkerArray(debug_data_.ego_lane_polygon, "ego_lane", lane_id_, 0.3, 0.0, 0.0),
    current_time, &debug_marker_array);

  appendMarkerArray(
    createPolygonMarkerArray(
      debug_data_.stuck_vehicle_detect_area, "stuck_vehicle_detect_area", lane_id_, 0.3, 0.0, 0.0),
    current_time, &debug_marker_array);
  ...
  for (const auto & p : debug_data_.candidate_collision_object_polygons) {
    appendMarkerArray(
      createPolygonMarkerArray(
        p, "candidate_collision_object_polygons", lane_id_ + i++, 0.3, 0.0, 0.0),
      current_time, &debug_marker_array);
  }
