  ...
  tier4_planning_msgs::msg::StopReason * stop_reason)
{
  ...
  *stop_reason =
  ...
  const bool is_stuck = checkStuckVehicleInIntersection(
    lanelet_map_ptr, *path, closest_idx, stuck_line_idx, objects_ptr);
  ...
  if (!isActivated()) {
    ...
    util::setVelocityFrom(stop_line_idx_final, v, path);
  ...
  const int closest_idx)
{
  ...
  for (const auto & object : objects_ptr->objects) {
    ...
    if (intersection_area) {
      // obj2polygon(predicted_pose, object.shape.dimensions)
      const auto obj_poly = obj2polygon(object_pose, object.shape.dimensions);
      const auto intersection_area_2d = intersection_area.value();
